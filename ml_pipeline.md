...
### 設計思想・結果管理ルール
- 環境構築
    - Pythonの仮想環境はvenvで管理する
    - 使用するPythonのライブラリとそのバージョンをrequirements.txtに記述し，これに基づきライブラリをインストールする
    - 学習結果を保存するディレクトリやデータセットのディレクトリはサイズが巨大のため.gitignoreに追加する

- デバック性の向上
    - 関数を定義する場合は，その関数の概要，引数のデータ型や形状，戻り値のデータ型や形状をコメントで明記する
    - クラスを定義する場合は，そのクラスの概要を
...
- 学習の実行
    - Modelクラスを記述したmodel.pyやload_dataloader関数等をまとめたutils.pyは，以下の規則でディレクトリを作成し，保存してください
      ...
      ex) `mnist/cnn`
    - 学習はプログラム終了後もログを残すために，`{data_name}/{model_name}/train.ipynb`で実行してください
...
- 学習結果の可視化
    ...
    - matplotでグラフを作成する場合は正方形にしてください．この際，私は良くfigsize = (5, 5)と設定しますが，文字がはみ出る等の問題がある場合，この限りではありません
