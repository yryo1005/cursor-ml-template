...
- プログラムはipynbファイルで実行するため，いずれの環境も必ずipykernelをインストールする．
- 複数の仮想環境を作る場合があるため環境名は適時設定する
- 使用するライブラリとそのバージョンを `{環境名}_requirements.txt` に記述し，これに基づきインストールする．
...
# 4. AIの行動規範および生成プロセス制限
- **関数シグネチャの完全一致（厳守）**:
   - `load_dataloader`, `load_model`, `loss_func`, `metrics_func`, `iteration`, `epoch`, `train` の各関数を実装する際，本書の「2. 実装すべき共通関数・クラス仕様」に記載されている**引数の名前，デフォルト値，戻り値の形式**を大幅に変更することは禁止します．
   - これらの機能は`ex_{n:03}_{全ての比較手法で共通の条件}/utils.py`に統一できるはずです．ここに，記載し人間がコードを書くにする際の可読性を担保してください．ネストが深くなると人間のデバックが困難になるので，その様なプログラムは避けるべきでしょう．

- **成果物の2段階生成ルール（report.mdの作成保証）**:
   - あなたのタスクは，コード（`model.py`, `utils.py`, `train.ipynb`）を生成して終了ではありません．
   - 実験環境の構築指示を受け取った段階で，メインロジックの依存関係を解説した `document.md` をルート直下に必ず自動生成してください．
   - また，実験結果を記録するための雛形，またはすべての実験が終了した段階で結果とグラフに基づく考察をまとめるための `report.md` (または `report_{k}.md`) をルート直下に必ず作成（または更新）してください．これらを怠った場合はタスク未完了とみなします．

- **TeXレポートの作成**
    - TeXレポートはcursorの終了時に作成する`report.md`とは別物で，人間からの指示があった場合にのみ作成してください
    - TeXレポートの作成が依頼された場合，`cursor-ml-template/TeX_template`内のTeXファイル等を編集し，PDFのレポートを作成してください
    - この際 `ieicejsp.cls` を変更しないでください．
...
```text
.
├── cursor-ml-template/      # cursorへの指示等やTeXレポートのテンプレートをまとめたリポジトリ
│   ├── ml_pipeline.md       # プログラムの設計思想や，実験する上での指示が書かれたドキュメント
│   ├── ml_utils.py          # 実験結果のロガー等の便利関数が定義されている
|   └── TeX_template/        # TeXレポートのテンプレート
